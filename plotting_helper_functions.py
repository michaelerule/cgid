...
def ppc(session,area,unit,start,stop,window=100,fmax=250,color='k',label=None,nTapers=None,lw=1.5,linestyle='-'):
...
def plotLFPSpikes(lfp,start,stop,spikes,zoom=1000,vrange=100,FS=1000.0,SPIKECOLOR=(1,0.5,0),lw=2):
    print 'caution assumes starting at beginning of trial'
    cla()
    plot((arange(stop-start)/FS)[:zoom],lfp[:zoom],'k',label='%s-%sHz LFP'%(lowf,highf))
    a,b = ylim()
    ylim(-100,100)
    a,b = ylim()
...
def plotISIhistHz(session,area,unit,start,stop,FS=1000,style='bar',color='k',nbins=30,label=None,linestyle='-',lw=2):
    ...
    isi = FS/array(isi)
    if style=='bar':
        hist(isi,linspace(0,100,nbins+1),facecolor='k')
        a,b = ylim()
...
def getSpec(session,area,unit,trial,epoch,lowf,highf):
    ...
    channeldata = get_raw_lfp(session,area,ch,trial,epoch)
    betadata    = get_filtered_lfp(session,area,ch,trial,epoch,lowf,highf)
    return betadata,fft_cwt(channeldata,lowf,highf,w=10.0)
